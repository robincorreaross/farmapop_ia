"""
updater.py - Verificador e instalador automático de atualizações do FarmaPop IA.

Fluxo de auto-update no Windows:
 1. Baixa FarmaPop_IA.zip para %TEMP%
 2. Extrai para %TEMP%/farma_update/
...
def verificar_atualizacao(
    ...
    def _check() -> None:
        try:
            req = request.Request(
                UPDATE_URL,
                headers={"User-Agent": f"FarmaPop_IA/{APP_VERSION}"},
            )
...
def baixar_e_instalar(
    ...
    """
    def _run() -> None:
        tmp_dir = Path(tempfile.mkdtemp(prefix="farma_update_"))
        zip_path = tmp_dir / "FarmaPop_IA.zip"
        ...
        try:
            ...
            # ── 3. Cria o bat helper ──────────────────────────────────────────
            app_dir = get_app_dir()
            app_exe = app_dir / "FarmaPop_IA.exe"
            bat_path = tmp_dir / "update_helper.bat"

            bat_content = f"""@echo off
title FarmaPop IA - Instalando Atualizacao...
